...
- Status: active close scope split out; use `_codex/tasks/050_ACTIVE_CLOSE_BRIEF.md` for remaining work
...
## Active Routing - 2026-06-26

Do not use this full PRD as the default working context for the final close
pass. It now contains historical design and implementation notes that are too
large for normal coding prompts.

Use these smaller files instead:

- remaining scope for Coding Agent: `_codex/tasks/050_ACTIVE_CLOSE_BRIEF.md`
- completed-state summary: `_codex/tasks/050_COMPLETED_STATE.md`
- detailed QA/protocol history: `_codex/tasks/QA/2026-06-25_2201_050_CodingAgentCompletion.md`
- detailed QA/protocol history: `_codex/tasks/QA/2026-06-25_2202_050_QA_ArchitectureReview.md`
- detailed QA/protocol history: `_codex/tasks/QA/2026-06-25_2203_050_CodingAgentFollowup.md`
- detailed QA/protocol history: `_codex/tasks/QA/2026-06-25_2204_050_QA_FinalArchitectureReview.md`
- detailed QA/protocol history: `_codex/tasks/QA/2026-06-26_0852_050_CodingAgentFollowup2.md`
- detailed QA/protocol history: `_codex/tasks/QA/2026-06-26_0852_050_QA_Followup2Review.md`

Open this parent PRD only when the active close brief conflicts with code or is
ambiguous.
...
## Implementation Follow-Up - 2026-06-26

### What Changed

- Removed legacy compatibility fields and factories from `TacticalAIPlannedAction`, `TacticalAISearchPlan`, `TacticalAIExecutionAttempt`, and `TacticalAIExecutionResult`.
- `TacticalAIExecutionBridge` now accepts and executes planned `BattleAction` payloads only for non-skill actions; it no longer falls back to direct non-skill `MouseControler.TryStart*` calls.
- Fallback planning now regenerates fresh legal `BattleAction` candidates through `BattleActionRules.GenerateLegalActions(...)`.
- `TacticalAISnapshotProbe` now builds plans with `TacticalAISearchPlanner.BuildPlan(...)` and executes `OrderedActions`.
- Updated focused EditMode tests to assert `BattleActionUse` / `BattleAction` payloads instead of legacy intent payloads.

### Automatic Test

- Unity compile and Unity Test Runner were not run automatically per project rules.
- Source audit passed for removed runtime symbols:
  - `LegacyIntent`
  - `ExecutedIntent`
  - `BestIntent`
  ...
  - `FromLegacyIntent`
  - `FromCandidateIntent`
- Brace-balance checks passed for changed C# files.

### Still Open For Full PRD050

- `BattleActionLiveApplier` still delegates live non-skill commits to `MouseControler.TryStartMoveAction`, `TryStartMoveAndAttackAction`, `TryStartBasicRangedAttackAction`, `TryStartWaitAction`, and `TryStartDefenseAction`.
- `TacticalAIActionIntent`, `TacticalAICandidateGenerator`, `TacticalAISearchCandidateExpander`, and `TacticalAIIntentRevalidator` still exist in AI search/candidate internals and tests.
- `CastManager`, `SkillUse`, `SkillCast`, `SkillResult`, and skill-only `SkillRules` runtime surfaces still exist.
- Player input is not fully migrated to `BattleActionUse`.
- AI search simulation is not fully converted to direct `BattleActionResult` pure apply.

### Next Steps

- Run Unity import/compile.
- Run EditMode tests for `BattleActionRulesTests`, `TacticalAIExecutionBridgeTests`, `TacticalAISearchScoringTests`, `TacticalAIAsyncDecisionPipelineTests`, and `TacticalAILiveTurnIntegrationTests`.
- Next PRD050 slice should replace `BattleActionLiveApplier` with result-event application so `MouseControler` is no longer execution authority for AI non-skill actions.

## Implementation Follow-Up 2 - 2026-06-26

### What Changed

- Deleted the legacy Tactical AI intent/candidate/revalidator runtime files:
  - `TacticalAIActionIntent.cs`
  - `TacticalAICandidateGenerator.cs`
  - `TacticalAIIntentRevalidator.cs`
- Replaced `TacticalAISearchCandidateExpander` usage with direct `BattleActionRules.GenerateLegalActions(...)` search candidates.
- `TacticalAISearchScoring` now scores `BattleAction` candidates and simulates through `BattleActionRules.Apply(...)` result events.
- `TacticalAIExecutionBridge` revalidates all planned actions, including skills, through `BattleActionRules.Validate(BattleActionUse, BattleSnapshot, ...)`.
- `TacticalAIPlannedAction` no longer exposes `SubmittedSkillUse`, `PreviewResult`, or `FromSkill(...)`.
- `BattleActionLiveApplier` applies non-skill `BattleActionResult` events directly instead of delegating to `MouseControler.TryStart*`.
- `MouseControler.TryStartMoveAction`, `TryStartMoveAndAttackAction`, `TryStartBasicRangedAttackAction`, `TryStartWaitAction`, and `TryStartDefenseAction` now act as player/legacy input adapters that submit `BattleActionUse` and execute through `BattleActionLiveApplier`.
- Updated EditMode tests to assert `BattleAction` legal action generation, search scoring, and bridge revalidation instead of deleted intent/generator/revalidator classes.

### Automatic Test

- Unity compile and Unity Test Runner were not run automatically per project rules.
- Source audit passed for removed AI intent symbols:
  - `TacticalAIActionIntent`
  - `TacticalAICandidateGenerator`
  - `TacticalAISearchCandidateExpander`
  - `TacticalAIIntentRevalidator`
  ...
  - `PreviewResult`
  - `FromSkill(`
- Brace-balance checks passed for changed files without disabled preprocessor legacy blocks; `MouseControler` contains one `#if false` block around the removed defense fallback body because of legacy text encoding in that block.

### Still Open For Full PRD050

- `CastManager`, `SkillUse`, `SkillCast`, `SkillResult`, `SkillQuery`, and the skill-only `SkillRules` runtime surface still exist.
- Active skill execution still flows through `TacticalAISkillRulesExecutor` / `SkillCast` internals after `BattleActionRules` validation.
- Passive/trap/automatic action migration is not complete.
- `MouseControler.TryStart*` methods still exist as compatibility entry points, although non-skill bodies now submit `BattleActionUse`.
- Manual Unity compile, EditMode tests, and Play Mode parity checks are still required before closure.
